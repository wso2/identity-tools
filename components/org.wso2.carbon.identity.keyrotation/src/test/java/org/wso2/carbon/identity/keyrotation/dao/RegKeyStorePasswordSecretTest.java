...
public class RegKeyStorePasswordSecretTest extends BaseSecretTest {
    ...
    public void setDatabaseQueries() {
        ...
        this.insertDataQuery = "INSERT INTO REG_PROPERTY" +
                ...
                "VALUES(23, '" + KeyRotationConstants.REGISTRY_PASSWORD + "', '"
                + encryptedSecrets.get("regKeyStorePassword") + "', 3), (24, 'current-theme','Default', 3);";
    }
    ...
    public void performAssertions() throws KeyRotationException {
        ...
        DBKeyRotator.getInstance().reEncryptKeystorePasswordData(keyRotationConfig);
        List<RegistryProperty> reEncryptedSecrets =
                RegistryDAO.getInstance().getRegPropertyDataChunks(0, keyRotationConfig,
                        KeyRotationConstants.REGISTRY_PASSWORD);
        // Assertions
        assertEquals(1, reEncryptedSecrets.size());

        assertEquals("23", reEncryptedSecrets.get(0).getRegId());
        assertEquals(KeyRotationConstants.REGISTRY_PASSWORD, reEncryptedSecrets.get(0).getRegName());
